...
export default defineComponent({
  ...
  methods: {
    async onConnect(connection) {
      const resp = await invoke("sqlite_connect", {
        dbPath: connection.filePath,
      });
      if (resp.connected) {
  ...
  <div class="flex flex-col gap-5 w-[400px] mt-20">
    <div v-if="connections.length > 0" class="flex flex-col gap-5">
      ...
      <button
        ...
        class="border border-zinc-700 p-2 flex items-center gap-5 hover:bg-zinc-800 cursor-pointer"
      >
        <Database />
        <p>{{ connection.name }}</p>
      </button>
      <button
        ...
        class="self-end w-[30%] p-2 bg-green-500 text-white cursor-pointer"
      >
        New connection
      </button>
    </div>
    ...
    <div v-else class="flex flex-col gap-5 border p-5 border-gray-100">
      <b class="flex items-center gap-2">
        <Ban :size="16" /> No saved connections found.</b
      >
    </div>

    <AddConnectionForm
      v-if="showAddForm"
      :showCancel="connections.length > 0"
