...
export default defineComponent({
  ...
  methods: {
    handleSave() {
      ...
      const conns =
        connsJSON && Array.isArray(JSON.parse(connsJSON))
          ? JSON.parse(connsJSON)
          : [];
      if (this.type === "sqlite") {
        conns.push({
          type: "sqlite",
...
<template>
  <div
    ...
    class="flex flex-col gap-5 w-[400px] border-zinc-700 border p-5"
  >
    ...
    <div class="flex flex-col gap-2">
      ...
      <div class="flex items-center gap-2">
        <button
          @click="type = 'sqlite'"
          :class="{ 'bg-zinc-600 border-none': type === 'sqlite' }"
          class="border px-5 h-8 border-zinc-700 cursor-pointer"
        >
          SQLite
        </button>
        <button
          @click="type = 'postgres'"
          :class="{ 'bg-zinc-600 border-none': type === 'postgres' }"
          class="border px-5 h-8 border-zinc-700 cursor-pointer"
        >
          PostgreSQL
        </button>
        <button
          @click="type = 'mysql'"
          :class="{ 'bg-zinc-600 border-none': type === 'mysql' }"
          class="border px-5 h-8 border-zinc-700 cursor-pointer"
        >
          MySQL
        </button>
    ...
    <slot v-if="type !== 'sqlite'">
      <div class="flex flex-col gap-2">
        ...
        <input
          ...
          type="text"
          class="border border-zinc-700 px-2 py-2 focus:outline-[1.2px] outline-green-500"
        />
      </div>
      <div class="flex flex-col gap-2">
    ...
    <div class="flex items-center">
      <button
        v-if="showCancel"
        @click="onCancel"
        class="h-8 cursor-pointer border border-zinc-700 w-[30%] flex justify-center items-center gap-3"
      >
        Cancel <span class="text-gray-400">Esc</span>
      </button>
      <!-- <button class="h-8 cursor-pointer ml-auto bg-gray-200 w-[20%]">Test</button> -->
      <button
        @click="handleSave"
        class="ml-auto h-8 cursor-pointer flex justify-center items-center gap-2 text-white bg-green-500 w-[30%] font-bold"
      >
        Save
      </button>
